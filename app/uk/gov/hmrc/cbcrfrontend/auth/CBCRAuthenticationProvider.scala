...
import uk.gov.hmrc.cbcrfrontend.views.subscription._

import scala.concurrent.{ExecutionContext, Future}
import uk.gov.hmrc.cbcrfrontend.views.html._
import uk.gov.hmrc.cbcrfrontend.views.html.subscription.{notAuthorisedAgent, notAuthorisedIndivitial}
...
class SecuredActionsImpl @Inject()(configuration: Configuration)(implicit cache: CBCSessionCache, val authConnector: AuthConnector, ec: ExecutionContext) extends SecuredActions {
  ...
  private val IndividualAuth = AuthenticatedBy(new CBCRAuthenticationProvider(configuration), IndividualUserVisibility)
  ...
  override def AsyncAuthenticatedAction(u: Option[UserType] = None)(r: AsyncUserRequest): Action[AnyContent] = u match {
...
class AffinityGroupPredicate(restrictToUser: UserType)(implicit auth: AuthConnector, cache: CBCSessionCache, ec: ExecutionContext)
  ...
  private def errorPage(implicit request: Request[_]) = Future.successful(Unauthorized(notAuthorisedAgent(includes.asideBusiness(), includes.phaseBannerBeta())))
...
object IndividualUserVisibility extends PageVisibilityPredicate{
  override def apply(auth: AuthContext, request: Request[AnyContent]): Future[PageVisibilityResult] = {
    implicit val r = request
    Future.successful(
      PageBlocked(Future.successful(Unauthorized(notAuthorisedIndivitial(includes.asideBusiness(), includes.phaseBannerBeta())))
      ))
  }
}